...
const badgeVariants = cva(
  "inline-flex shrink-0 items-center justify-center rounded-full whitespace-nowrap tabular-nums",
  {
    variants: {
      variant: {
        count:
          "h-(--badge-height) min-w-(--badge-min-width) bg-system-red px-1.5 type-footnote font-medium text-white",
        tag: "h-(--badge-height) min-w-(--badge-min-width) bg-primary/15 px-2 type-caption-1 font-semibold text-primary",
        filled:
          "h-(--badge-height) min-w-(--badge-min-width) bg-primary px-2 type-caption-1 font-semibold text-primary-foreground",
      },
    },
    defaultVariants: {
      variant: "count",
