 ...
 */
const pageControlVariants = cva('inline-flex items-center gap-(--page-control-gap)', {
  variants: {
    background: {
      automatic: 'rounded-full px-2 py-1',
      prominent: 'material-thin rounded-full px-2 py-1',
      minimal: '',
    },
  },
  defaultVariants: {
...
function PageControl({ className, background = 'automatic', count, index, onIndexChange, onKeyDown, ...props }: PageControlProps) {
    ...
    >
      {Array.from({ length: count }, (_, i) => (
        <button
          ...
          className="size-(--page-control-dot) rounded-full bg-label-4 transition-colors duration-(--duration-press) outline-none aria-selected:bg-label focus-visible:ring-3 focus-visible:ring-ring/50"
          onClick={() => move(i)}
        />
      ))}
    </div>
