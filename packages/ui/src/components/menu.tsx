  ...
  return (
    <MenuPrimitive.Portal>
      <MenuPrimitive.Positioner className="isolate z-50 outline-none" align={align} alignOffset={alignOffset} side={side} sideOffset={sideOffset}>
        <MenuPrimitive.Popup data-slot="menu-content" data-elevated="" className={cn(menuContentClassName, className)} {...props} />
...
function MenuLabel({ className, ...props }: MenuPrimitive.GroupLabel.Props) {
  return <MenuPrimitive.GroupLabel data-slot="menu-label" className={cn('type-footnote px-4 py-2 text-label-2', className)} {...props} />
}
...
function MenuItem({ className, variant = 'default', icon, children, ...props }: MenuItemProps) {
  return (
    <MenuPrimitive.Item data-slot="menu-item" data-variant={variant} className={cn(menuItemClassName, className)} {...props}>
...
function MenuCheckboxItem({ className, children, ...props }: MenuPrimitive.CheckboxItem.Props) {
  return (
    <MenuPrimitive.CheckboxItem data-slot="menu-checkbox-item" className={cn(menuItemClassName, 'ps-2', className)} {...props}>
      <span data-slot="menu-checkbox-item-indicator" className="flex w-5 shrink-0 items-center justify-center text-primary">
        <MenuPrimitive.CheckboxItemIndicator render={<Icon icon={Tick02Icon} weight="bold" />} />
      </span>
      {children}
...
function MenuRadioItem({ className, children, ...props }: MenuPrimitive.RadioItem.Props) {
  return (
    <MenuPrimitive.RadioItem data-slot="menu-radio-item" className={cn(menuItemClassName, 'ps-2', className)} {...props}>
      <span data-slot="menu-radio-item-indicator" className="flex w-5 shrink-0 items-center justify-center text-primary">
        <MenuPrimitive.RadioItemIndicator render={<Icon icon={Tick02Icon} weight="bold" />} />
      </span>
      {children}
...
function MenuSeparator({ className, ...props }: MenuPrimitive.Separator.Props) {
  return <MenuPrimitive.Separator data-slot="menu-separator" className={cn('-mx-1 my-1 h-2 bg-fill-4', className)} {...props} />
}

function MenuShortcut({ className, ...props }: ComponentProps<'span'>) {
  return <span data-slot="menu-shortcut" className={cn('type-body ms-auto text-label-2', className)} {...props} />
}
...
function MenuSubTrigger({ className, children, ...props }: MenuPrimitive.SubmenuTrigger.Props) {
  return (
    ...
    >
      {children}
      <Icon icon={ArrowRight01Icon} weight="semibold" className="ms-auto text-label-3" />
    </MenuPrimitive.SubmenuTrigger>
  )
}

function MenuSubContent({ align = 'start', alignOffset = -4, side = 'right', sideOffset = 0, ...props }: ComponentProps<typeof MenuContent>) {
  return <MenuContent data-slot="menu-sub-content" align={align} alignOffset={alignOffset} side={side} sideOffset={sideOffset} {...props} />
}
