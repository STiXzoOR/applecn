...
function NavigationBar({ className, title, largeTitle = false, leading, trailing, children, ...props }: NavigationBarProps) {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div data-slot="navigation-bar-leading" className="flex items-center gap-2 justify-self-start">
            {leading}
          </div>
          <div
            ...
            className={cn(
              'type-headline min-w-0 truncate text-center text-label transition-opacity duration-(--duration-nav)',
              showLargeTitle ? 'opacity-0' : 'opacity-100',
            )}
          >
            {title}
          </div>
          <div data-slot="navigation-bar-trailing" className="flex items-center gap-2 justify-self-end">
            {trailing}
          </div>
        </div>
        {largeTitle ? (
          <>
            <h1
              ...
              className="type-large-title flex min-h-(--nav-bar-large-title) items-end truncate px-4 pb-2 font-bold text-label"
            >
              {title}
            </h1>
            <div ref={ref} data-slot="navigation-bar-sentinel" aria-hidden="true" className="h-px" />
          </>
        ) : null}
      </header>
      {children}
...
type NavigationBarBackButtonProps = Omit<ComponentProps<'button'>, 'children'> & {
  href?: string
  label?: string
}

function NavigationBarBackButton({ className, href, label = 'Back', onClick, ...props }: NavigationBarBackButtonProps) {
  const buttonClassName = cn(
    'glass pressable flex size-(--control-height-regular) items-center justify-center rounded-full text-primary outline-none focus-visible:ring-3 focus-visible:ring-ring/50',
    className,
  )
  if (href) {
    return (
      <a href={href} aria-label={label} data-slot="navigation-bar-back" className={buttonClassName}>
        <Icon icon={ArrowLeft01Icon} weight="semibold" scale="large" />
      </a>
    )
  }
  return (
    <button type="button" aria-label={label} data-slot="navigation-bar-back" className={buttonClassName} onClick={onClick} {...props}>
      <Icon icon={ArrowLeft01Icon} weight="semibold" scale="large" />
    </button>
  )
}
