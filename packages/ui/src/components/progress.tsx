...
function Progress({ className, children, value, ...props }: ProgressPrimitive.Root.Props) {
  return (
    <ProgressPrimitive.Root value={value} data-slot="progress" className={cn('flex flex-wrap gap-2', className)} {...props}>
      {children}
      <ProgressTrack>
        <ProgressIndicator />
      </ProgressTrack>
...
function ProgressTrack({ className, ...props }: ProgressPrimitive.Track.Props) {
  return (
    <ProgressPrimitive.Track
      ...
      className={cn('relative flex h-(--progress-height) w-full items-center overflow-hidden rounded-full bg-fill-2', className)}
      {...props}
    />
  )
}
...
function ProgressLabel({ className, ...props }: ProgressPrimitive.Label.Props) {
  return <ProgressPrimitive.Label data-slot="progress-label" className={cn('type-footnote text-label', className)} {...props} />
}
...
function ProgressValue({ className, ...props }: ProgressPrimitive.Value.Props) {
  return (
    <ProgressPrimitive.Value
      ...
      className={cn('type-footnote ms-auto text-label-2 tabular-nums', className)}
      {...props}
    />
  )
}

const progressCircularVariants = cva('inline-block shrink-0 -rotate-90 text-primary', {
  variants: {
    size: {
...
function ProgressCircular({ className, size = 'medium', value, ...props }: ProgressCircularProps) {
  ...
  return (
    <svg
      ...
      {...props}
    >
      <circle data-slot="progress-circular-track" cx="18" cy="18" r="15" strokeWidth="3" className="stroke-fill-2" />
      <circle
...
export { Progress, ProgressCircular, ProgressIndicator, ProgressLabel, ProgressTrack, ProgressValue, progressCircularVariants }
