...
describe('iOS metrics', () => {
  ...
  test('button heights per control size', () => {
    expect(ios.buttonHeight).toEqual({ mini: 28, small: 32, regular: 44, large: 52, xl: 64 })
  })
...
describe('motion', () => {
  test('easings and durations', () => {
    ...
    expect(durations).toEqual({ press: 120, hover: 150, overlay: 250, nav: 300, sheet: 450 })
  })
  ...
  test('a bouncy spring overshoots and a smooth one does not', () => {
    const values = (s: string) => s.slice('linear('.length, -1).split(', ').map(Number)
...
describe('materials and elevation', () => {
  test('content materials thicken from ultra-thin to thick', () => {
    expect(materials['ultra-thin'].light.alpha).toBeLessThan(materials.thin.light.alpha)
    expect(materials.thin.light.alpha).toBeLessThan(materials.regular.light.alpha)
  ...
  test('glass has a regular and a clear variant', () => {
    expect(materials.glass.light.alpha).toBeGreaterThan(materials['glass-clear'].light.alpha)
  })
  test('shadows are CSS box-shadow strings', () => {
    expect(elevation.thumb).toBe('0 3px 8px rgb(0 0 0 / 0.15), 0 3px 1px rgb(0 0 0 / 0.06)')
