...
import { dynamicType, fontFamilies, iosTextStyles, macosTextStyles, tracking } from '../typography'

const byName = (styles: typeof iosTextStyles, name: string) => styles.find((s) => s.name === name)!
...
describe('iOS text styles at the Large (default) size', () => {
  ...
  ])('%s is %s/%s weight %s (emphasized %s)', (name, size, leading, weight, emphasized) => {
