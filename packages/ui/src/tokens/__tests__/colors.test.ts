...
describe("system colours", () => {
  ...
  test.each(table)(
    "%s matches the HIG table",
    (name, light, dark, lightA11y, darkA11y) => {
...
describe("grays", () => {
  test("the six-step ladder in light and dark", () => {
    expect(grays.map((g) => g.name)).toEqual([
      "gray",
      "gray-2",
      "gray-3",
      "gray-4",
...
describe("semantic colours", () => {
  ...
  test("backgrounds, with the elevated dark set one step lighter", () => {
    ...
    expect(css(groupedBackgrounds["grouped-background"].light)).toBe(
      "rgb(242 242 247)"
    )
    expect(css(groupedBackgrounds["grouped-background-2"].light)).toBe(
      "rgb(255 255 255)"
    )
    expect(css(groupedBackgrounds["grouped-background-2"].dark)).toBe(
      "rgb(28 28 30)"
    )
    expect(css(groupedBackgrounds["grouped-background-3"].darkElevated)).toBe(
      "rgb(58 58 60)"
    )
  })
