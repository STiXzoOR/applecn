...
function primitives(appearance: Appearance): Line[] {
  ...
  lines.push(['grouped-background-1', pick(groupedBackgrounds['grouped-background'])])
  lines.push(['grouped-background-2', pick(groupedBackgrounds['grouped-background-2'])])
  lines.push(['grouped-background-3', pick(groupedBackgrounds['grouped-background-3'])])
  for (const [name, v] of Object.entries(separators)) lines.push([name, pick(v)])
...
function elevatedPrimitives(): Line[] {
  return [
    ...
    ['grouped-background-1', css(groupedBackgrounds['grouped-background'].darkElevated)],
    ['grouped-background-2', css(groupedBackgrounds['grouped-background-2'].darkElevated)],
    ['grouped-background-3', css(groupedBackgrounds['grouped-background-3'].darkElevated)],
  ]
}
...
function semanticAliases(appearance: Appearance): Line[] {
  return [
    ...
    ['popover', appearance === 'dark' ? 'var(--background-2)' : 'var(--background-1)'],
...
export function tokenVars(appearance: Appearance): Record<string, string> {
  const lines: Line[] =
    appearance === 'light'
      ...
      : [...primitives('dark'), ...semanticAliases('dark'), ...materialLines('dark')]
  return Object.fromEntries(lines)
}

export function renderTokensCss(): string {
  const parts: string[] = []
  parts.push('/* Generated by scripts/build-css.ts from src/tokens — edit the token modules, then run `pnpm tokens:build`. */')
  ...
  parts.push(`@media (width >= 414px) {\n${block(':root', [['list-inset', px(metrics.ios.list.insetWide)]], '  ')}\n}`)

  parts.push(block('.dark', [...primitives('dark'), ...semanticAliases('dark'), ...materialLines('dark')]))
  parts.push(block('.dark [data-elevated]', [...elevatedPrimitives(), ...semanticAliases('dark')]))

  const contrastLight = [...accessiblePrimitives('light'), ...semanticAliases('light')]
  const contrastDark = [...accessiblePrimitives('dark'), ...semanticAliases('dark')]
