  ...
  test('is a floating glass capsule of labelled tabs with the current one tinted', () => {
    render(
      <TabBar aria-label="Main" value="home">
        ...
        <TabBarItem value="you" icon={UserIcon} label="You" href="/you" badge={3} />
    ...
    expect(screen.getByRole('link', { name: 'Search' })).toHaveAttribute('data-slot', 'tab-bar-search')
  })

  test('button items select through onValueChange, and minimized shows only the current tab', async () => {
    const onValueChange = vi.fn()
    render(
      <TabBar aria-label="Main" value="home" onValueChange={onValueChange} minimized>
