...
function Draft() {
  return (
    <ActionSheet>
      ...
      <ActionSheetContent title="Unsaved Draft" message="You can save it for later.">
...
describe('ActionSheet', () => {
  test('on a phone it rises from the bottom: a titled group of 56 pt rows and a separate Cancel', async () => {
    ...
    const cancelGroup = sheet.querySelector('[data-slot="action-sheet-cancel-group"]')!
    expect(cancelGroup.className).toContain('mt-(--action-sheet-cancel-gap)')
    expect(cancelGroup).toContainElement(screen.getByRole('button', { name: 'Cancel' }))
    expect(screen.getByRole('button', { name: 'Cancel' }).className).toContain('font-semibold')
  })
  ...
  test('on a desktop it is a popover anchored to the trigger', async () => {
    ...
    expect(sheet.querySelector('[data-slot="action-sheet-cancel-group"]')).toBeNull()
  })
