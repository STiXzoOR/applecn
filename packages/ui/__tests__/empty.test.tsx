...
import { Empty, EmptyActions, EmptyDescription, EmptyIcon, EmptyTitle } from '../src/components/empty'
...
describe('Empty (ContentUnavailableView)', () => {
  test('centres a large symbol, a bold title and a secondary description', () => {
    ...
    expect(screen.getByText('Check the spelling or try a new search.').className).toContain('text-label-2')
    expect(screen.getByRole('button', { name: 'Clear' }).closest('[data-slot="empty-actions"]')).not.toBeNull()
  })
})
