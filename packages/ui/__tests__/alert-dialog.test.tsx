    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        ...
        <AlertDialogActions>
          ...
          {props.third ? <AlertDialogAction>Move to Trash</AlertDialogAction> : null}
...
describe('AlertDialog', () => {
  test('is a 270 pt alert on thick material with left-aligned text and 44 pt buttons', async () => {
    ...
    const alert = await screen.findByRole('alertdialog', { name: 'Delete Note?' })
    ...
    expect(screen.getByRole('button', { name: 'Cancel' }).className).not.toContain('font-semibold')
  })

  test('two short actions sit side by side; three stack', async () => {
    const { unmount } = render(<DeleteNote />)
    await userEvent.click(screen.getByRole('button', { name: 'Delete' }))
    expect((await screen.findByRole('alertdialog')).querySelector('[data-slot="alert-dialog-actions"]')).toHaveAttribute(
      'data-layout',
      'horizontal',
    )
