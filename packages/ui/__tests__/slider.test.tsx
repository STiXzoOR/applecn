...
describe('Slider', () => {
  ...
  test('is a 4 pt track with a 28 pt white thumb on iOS, from the platform tokens', () => {
    const { container } = render(<Slider aria-label="Brightness" defaultValue={50} />)
  ...
  test('minimum and maximum value labels flank the track', () => {
    ...
    expect(screen.getByTestId('min').closest('[data-slot="slider-minimum-label"]')).not.toBeNull()
    expect(screen.getByTestId('max').closest('[data-slot="slider-maximum-label"]')).not.toBeNull()
  })
})
