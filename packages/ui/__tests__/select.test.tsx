...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue, selectTriggerVariants } from '../src/components/select'
...
describe('Select (menu picker)', () => {
  ...
  test('the plain trigger reads as a tinted label with chevrons; the popup style is a macOS pop-up button', () => {
    expect(selectTriggerVariants({ variant: 'plain' })).toContain('text-primary')
    expect(selectTriggerVariants({ variant: 'popup' })).toContain('bg-fill-3')
    expect(selectTriggerVariants({ variant: 'popup' })).toContain('h-(--text-field-height)')
    render(<Fruit />)
    expect(screen.getByRole('combobox').querySelector('[data-slot="select-trigger-icon"]')).not.toBeNull()
  })
})
