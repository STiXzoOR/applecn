...
- [x] **Step 1: Replace the scope everywhere**
...
- [x] **Step 2: Root scripts and turbo tasks**
...
- [x] **Step 3: Verify install and typecheck**

Run: `pnpm install && pnpm typecheck`
Expected: both workspaces typecheck.

- [x] **Step 4: Commit** `chore(repo): rename scope to @apple-ds, root scripts`
...
- [x] **Step 2: Run it** — `pnpm --filter @apple-ds/ui test` — Expected: fails, no vitest script.
...
- [x] **Step 4: Run** — Expected: PASS. **Step 5: Commit** `test(ui): vitest harness`
...
- [x] **Step 1: Failing tests** (fixtures are the HIG "Specifications" table, fetched 2026-09-05)
...
- [x] **Step 2: Run** — fails (module missing). **Step 3: Implement** the module with every value from the research doc. **Step 4: Run** — PASS. **Step 5: Commit** `feat(tokens): Apple colour tokens`
...
- [x] Failing tests: `body` is 17/22 regular emphasized semibold; `large-title` 34/41 regular emphasized bold; macOS `headline` 13/16 bold; `dynamicType.AX5` large-title is 60/70; `tracking` at 17 is −26 and at 34 is +12; `fontFamilies.sans` starts with `-apple-system`. Then implement, run, commit `feat(tokens): typography`.
...
test("renders the light system colours on :root", () => {
  expect(renderTokensCss()).toContain("--system-blue: rgb(0 136 255);")
})
test("renders dark values under .dark", () => {
  expect(renderTokensCss()).toMatch(
    /\.dark\s*{[^}]*--system-blue: rgb\(0 145 255\);/
  )
})
test("renders accessible values under prefers-contrast", () => {
  expect(renderTokensCss()).toMatch(
    /prefers-contrast: more[^]*--system-blue: rgb\(30 110 244\);/
  )
})
test("renders macOS type sizes under [data-platform=macos]", () => {
  expect(renderTokensCss()).toMatch(
    /\[data-platform="macos"\][^]*--type-body-size: calc\(13 \* var\(--pt\)\);/
  )
})
...
- [x] Tests: `PlatformProvider platform="macos"` renders `data-platform="macos"` on its wrapper and `usePlatform()` returns `'macos'`; default is `'ios'`; `contrastRatio({rgb:[255,255,255]}, [0,136,255])` is `3.5 ± 0.05`; `contrastRatio(labels['label-2'].light, [255,255,255])` is `3.4 ± 0.1`; a `token-contrast` test asserts `foreground` on `background` ≥ 4.5 in light and dark and `primary-foreground` on `primary` ≥ 3 (Apple's rule for 17 pt semibold).
- [x] Commit `feat(ui): platform provider, contrast helper, research doc`
...
test.each([
  ...
  expect(buttonVariants({ variant: variant as never })).toContain(
    classes.split(" ")[0]
  )
})
test.each([
  ...
  expect(buttonVariants({ size: size as never })).toContain(
    `h-(--control-height-${size})`
  )
})
...
- [x] **Step 2: Run** `pnpm --filter @apple-ds/ui test button` — Expected: FAIL (`data-variant` missing, sizes missing).
...
const buttonVariants = cva(
  "group/button text-body inline-flex shrink-0 items-center justify-center gap-1.5 border border-transparent bg-clip-padding font-semibold whitespace-nowrap transition-[background-color,transform,opacity] duration-(--duration-press) ease-(--ease-standard) outline-none select-none focus-visible:ring-3 focus-visible:ring-ring/50 active:scale-[0.97] active:opacity-80 disabled:pointer-events-none disabled:opacity-40 motion-reduce:active:scale-100 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        mini: "text-caption-1 h-(--control-height-mini) px-2.5",
        small: "text-subheadline h-(--control-height-small) px-3",
        regular: "h-(--control-height-regular) px-5",
        large: "h-(--control-height-large) px-6",
        xl: "text-title-3 h-(--control-height-xl) px-7",
      },
      shape: {
...
- [x] **Step 4: Run** — PASS. **Step 5: Commit** `feat(ui): button with Apple styles, sizes and shapes`
