...
export default async function ComponentPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const source = await readFile(join(process.cwd(), '../../packages/ui/src/components', `${doc.name}.tsx`), 'utf8')
  ...
  return (
    <>
      <PageHeader title={doc.title} description={doc.description}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Text variant="footnote" color="label-2">
            Apple:{' '}
            {doc.apple.hig ? (
              <a href={doc.apple.hig} className="text-primary underline-offset-4 hover:underline" target="_blank" rel="noreferrer">
                {doc.apple.name}
              </a>
            ) : (
              doc.apple.name
      ...
      {await Promise.all(
        doc.examples.map(async (example) => {
          ...
          const [{ default: Example }, code] = await Promise.all([
            loader(),
            readFile(join(process.cwd(), 'registry/examples', `${key}.tsx`), 'utf8'),
          ])
          return (
            <Section key={key} id={example.name} title={example.title} description={example.description}>
              <Preview>
                <Example />
              </Preview>
              <CodeBlock code={code} />
      ...
      <Section title="Install" description="From the registry this site publishes, with dependencies resolved by the CLI.">
        <CodeBlock lang="bash" code={`npx shadcn@latest add https://<your-host>/r/${doc.name}.json`} />
      </Section>

      <Section title="Source" description={`packages/ui/src/components/${doc.name}.tsx`}>
