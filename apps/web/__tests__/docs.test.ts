...
const uiComponents = readdirSync(
...
describe("component documentation", () => {
  ...
  test("every example has a file and an entry in the generated map, and the map has nothing extra", () => {
    const expected = componentDocs
      .flatMap((d) => d.examples.map((e) => `${d.name}/${e.name}`))
      .sort()
    for (const key of expected) {
      expect(
        existsSync(join(process.cwd(), "registry/examples", `${key}.tsx`)),
        key
      ).toBe(true)
    }
...
describe("navigation", () => {
  test("lists every foundation page and every component, grouped", () => {
    ...
    for (const page of foundationPages)
      expect(hrefs).toContain(`/foundations/${page.slug}`)
    for (const doc of componentDocs)
      expect(hrefs).toContain(`/components/${doc.name}`)
