...
export function MaterialsPage() {
  return (
    <>
      ...
      />
      <Section title="Content layer" description="Thicker materials give text more contrast; thinner ones keep the background present.">
        <div className="grid grid-cols-2 gap-4 rounded-4xl bg-[linear-gradient(135deg,var(--system-pink),var(--system-yellow),var(--system-mint))] p-4 sm:grid-cols-4">
          {(['ultra-thin', 'thin', 'regular', 'thick'] as const).map((t) => (
            <Material key={t} thickness={t} className="flex h-28 flex-col items-center justify-center gap-1 rounded-3xl">
      ...
      <Section title="Liquid Glass" description="Regular keeps text legible over anything; clear shows a rich background and expects a dimming layer over bright content; prominent is the one tinted action.">
      ...
      <Section title="Values" description="Background tint per appearance, backdrop blur radius and saturation.">
        <TokenTable
          columns={['Material', 'Light', 'Dark', 'Blur', 'Saturate']}
          rows={Object.entries(materials).map(([n, m]) => [n, css(m.light), css(m.dark), `${m.blur}px`, `${m.saturate}`])}
        />
      </Section>
    </>
  )
